...
// ==/UserScript==
    ...
    var descriptions = ['Включить панель инструментов', 'Построение карты ответов', 'Скрытие постов', 'Панель смайлов',
                        ...
                        'Изменять имя страницы', 'Скрывать посты по иконке', 'Поиск картинок'
                       ];
...
function imageSearch() {
        ...
        var names = ['Google', 'TinEye', 'IQDB', 'SauceNao', 'EXIF' ];
        
        var files = document.getElementsByClassName('b-image-link');
        for (var i = 0; i < files.length; i++) {
            var imgSearch = document.createElement("div");
            imgSearch.id = "image-search";
            //imgSearch.style.minHeight = "5px";
            //imgSearch.style.margin = "1px";
            //imgSearch.style.paddingLeft = "5px";
            imgSearch.style.border = "1px solid #CCCCCC";
            imgSearch.style.borderRadius = "3px";
            
            var pic = files[i];
            var address = pic.href;
            for (var j = 0; j < names.length; j++) {
                var link = document.createElement('a');
                link.href = searchEngines[j] + address;
                link.style.fontSize = "12px"
                link.textContent = names[j];
                imgSearch.appendChild(link);
                imgSearch.appendChild(document.createTextNode(' '));
                }
            var par = files[i].parentNode;
            var res = files[i];
            par.removeChild(files[i]);
            //par.appendChild(document.createElement('br'));
            par.appendChild(imgSearch);
            par.appendChild(res);
            //imgSearch.setAttribute("style", 'top: 5px; left:5px; position:fixed; \z-index: 10000; background: #EAF4FF; padding: 5px');
            //files[i].appendChild(imgSearch);
        }
...
*/
    ...
    function createMenu() {
		...
		var intel = document.createElement("a");
        ...
        help.id = "help-button";
		intel.href = "#";
        //intel.onclick = displayIntelHiding;
        intel.id = "intel-button";
        if(enabledFeatures.indexOf('form-settings')!= -1) {
            ...
            helpIcon.src = "https://cdn1.iconfinder.com/data/icons/musthave/16/Help.png";
            help.appendChild(helpIcon);
			var intelIcon = document.createElement("img");
            intelIcon.src = "https://cdn0.iconfinder.com/data/icons/fugue/icon/brain.png";
            intel.appendChild(intelIcon);
			...
			intel.style.cssFloat = "right";
            intel.style.margin = "0px 2px 0px 2px";
            ...
            help.style.margin = "0px 2px 0px 2px";
			...
			help.title = "Справка";
			intel.title = "Система Автоскрытия";
			...
			container.parentNode.insertBefore(intel, container);
			...
			help.textContent = "Справка";
			intel.textContent = "Система автоскрытия";
...
*/
    ...
    function displayGeneralOptions() {
		...
		jQuery(container).append("<b>Общие настройки скрипта</b><br>");
	...
	function displayHelp() {
        var container = document.createElement("div");
        document.getElementById('help-button').onclick = hideHelp;
        container.id = 'help'
        container.setAttribute("style", 'top: 5px; left:5px; position:fixed; \
		z-index: 10000; background: #EAF4FF; padding: 5px');
		jQuery(container).append("<b>Помощь</b><br>");
		jQuery(container).append("<b>BL4H BL4H HUGE BITCH</b><br>");
        btn = document.createElement("button");
    ...
    function hideHelp() {
        var cont = document.getElementById('help');
        cont.parentNode.removeChild(cont);
        document.getElementById('help-button').onclick = displayHelp;
        return false;
    }
...
*/
    ...
    function letTheSobakOut() {
        ...
        jQuery(jQOmsk).append("<br>Режим Омска: <a href=\"http:\/\/1chan.ru\/service\/theme\/omsk\/\">Вкл</a>");
...
//jQuery(jQOmsk).append("<br>");
        ...
        if (formTextarea) {
            ...
            if(enabledFeatures.indexOf("spoilers")!= -1)
                revealSpoilers();
			if(enabledFeatures.indexOf("image-search")!= -1)
                imageSearch();
